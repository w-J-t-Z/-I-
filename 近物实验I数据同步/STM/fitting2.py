...
s1fitting[0,:]=pointmap[0,1,:]
s1fitting[1,:]=pointmap[0,0,:]

temp=2
for j in range(5):
    corx1=-pointmap[j,2,0]+s1fitting[temp-1,0]
    cory1=-pointmap[j,2,1]+s1fitting[temp-1,1]
    s1fitting[temp,0]=pointmap[j+1,1,0]+corx1
    s1fitting[temp,1]=pointmap[j+1,1,1]+cory1
    temp=temp+1
    s1fitting[temp,0]=pointmap[j+1,0,0]+corx1
    s1fitting[temp,1]=pointmap[j+1,0,1]+cory1
    temp=temp+1

s1fitting[:,0]=-s1fitting[:,0]

#fitting
a1x,a0x,rx=polyfit(np.arange(12),s1fitting[:,0])
a1x,a0x,rx=a1x[0],a0x[0],rx[0]
print(a1x,a0x,rx)

a1y,a0y,ry=polyfit(np.arange(12),s1fitting[:,1])
...
ax1.scatter(range(12),s1fitting[:,0],s=100,color='b',label="y experiment")
ax1.scatter(range(12),s1fitting[:,1],s=100,color='orange',label="x experiment")
ax1.plot([0,11],[a0x,a1x*11+a0x],lw=3,color='green',label="y fitting")
ax1.plot([0,11],[a0y,a1y*11+a0y],lw=3,color='purple',label='x fitting')
...
ax1.set_ylim(ymin = -100, ymax = 250)
